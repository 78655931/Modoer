...
class uc_note {
	...
	function uc_note() {
		...
		$this->ucconfig = $this->appdir.'./data/config_uc.php';
		$this->db = $GLOBALS['db'];
	...
	function updateapps($get, $post) {
		...
		//note 写配置文件
		if(is_writeable($this->ucconfig)) {
			$configfile = trim(file_get_contents($this->ucconfig));
			$configfile = substr($configfile, -2) == '?>' ? substr($configfile, 0, -2) : $configfile;
			$configfile = preg_replace("/define\('UC_API',\s*'.*?'\);/i", "define('UC_API', '$UC_API');", $configfile);
			if($fp = @fopen($this->ucconfig, 'w')) {
	...
	function updatecredit($get, $post) {
        ...
        if($q) {
            ...
            if(isset($r[$point])) {
                ...
                //兑换记录
                $this->db->exec("INSERT INTO {$this->dbpre}member_point_log(in_uid,in_username,in_point,in_value,dateline,des) 
                    VALUES($uid,'$r[username]','$point','$amount',$time,'用户积分兑换')");
            }
        } else {
            return API_RETURN_FORBIDDEN;
        }
	...
	function getcredit($get, $post) {
        ...
        $uid = intval($get['uid']);
        $credit = 'point' . intval($get['credit']);
