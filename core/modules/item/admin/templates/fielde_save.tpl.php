...
<form method="post" action="<?=cpurl($module, $act, $op)?>">
    <div class="space">
        <div class="subtitle">增加/编辑字段</div>
        <ul class="cptab">
            <li><a href="<?=cpurl($module,'model_list')?>" onfocus="this.blur()">模型管理</a></li>
            <li><a href="<?=cpurl($module,'model_edit','',array('modelid'=>$modelid))?>" onfocus="this.blur()">编辑模型</a></li>
            <li><a href="<?=cpurl($module,'field_list','',array('modelid'=>$modelid))?>" onfocus="this.blur()">自定义字段管理</a></li>
            <li class="selected"><a href="#" onfocus="this.blur()">新建字段</a></li>
        </ul>
        <table class="maintable" border="0" cellspacing="1" cellpadding="1">
            <tr>
                <td class="altbg1" width="45%"><strong>字段名称：</strong>此项在我们设计模板时将通过这个名称取得字段的值，由英文小写字母，数字和下划线组成，系统对自定义字段会默认增加前缀"c_“以防止与系统及数据库关键字重名。<span class="font_1">此项填写后将无法再次改动</span></td>
                <td width="*">
            ...
            <tr>
                <td class="altbg1"><strong>字段显示模板：</strong>可设置字段在前台默认显示的效果，多可用于特殊文字效果，IM工具在线图标等，变量如下：<br />{value}：字段的值<br />{urlcode:value}：进行URL加密的字段值，用于URL链接<br />
				{city_name}：城市名称<br />
				{display:函数}:支持模块的display类函数(core/modules/[module]/inc/disply.php)</td>
            ...
            <tr>
                <td class="altbg1"><strong>在内容页表显示：</strong>在主题内容页的信息表格中显示该字段信息，例如大型文字段，多行文本不适合显示在表格的字段，可以自行编辑模板，请不要将敏感字段信息显示。</td>
                <td><?=form_bool('t_field[show_detail]', $t_field['show_detail'])?></td>
            </tr>
            <tr>
                <td class="altbg1"><strong>在侧边栏显示：</strong>在关联主题的模块内容页显示主题信息，例如：产品内容页，优惠券内容页等；例如大型文字段，多行文本不适合显示在表格的字段，可以自行编辑模板，请不要将敏感字段信息显示。</td>
                <td><?=form_bool('t_field[show_side]', $t_field['show_side'])?></td>
            </tr>
            <tr>
                <td class="altbg1"><strong>字段编辑权限：</strong>设置当前字段在添加编辑权限；<br /><span class="font_1">注:无论选择哪一种类型，后台用户均可编辑所有字段.</span></td>
                <td><?=form_radio('t_field[isadminfield]', array(
                    '0'=>'前台普通字段<span class="font_2">(前台普通用户添加编辑时可见)</span>',
                    '2'=>'主题管理员字段<span class="font_2">(只有这个主题的管理员编辑时可见)</span>',
                    '1'=>'后台字段<span class="font_2">(仅系统后台用户可编辑)</span>'), 
