...
class task_item_subject {

    var $flag = 'item:subject';
    var $title = 'item_task_subtect_title';
    ...
    function form($cfg) {
        ...
        $result[] = array(
            'title' => '主分类限制',
            'des' => '设置会员只能在某个分类完成任务',
            'content' => '<select name="config[pid]">'.
                '<option value="0">=='.lang('global_unrestrained').'==</option>'.
                form_item_category_main($cfg['pid']).
                '</select>',
        );
        $result[] = array(
            'title' => '添加主题数量',
            'des' => '此任务要求会员添加主题的数量，默认为 1 条',
    ...
    function form_post($cfg) { 
        if(!$cfg['num'] || !is_numeric($cfg['num']) || $cfg['num'] < 1) redirect('完成条件错误：未填写一个有效的主题添加数量。');
    ...
    function progress(& $task_detail) {
        ...
        if($cfg['pid']>0) $db->where('pid',$cfg['pid']);
        $db->where('status', 1);
        $db->where('cuid', $_G['user']->uid);
        $db->where_more('addtime', $mytask['applytime']);
